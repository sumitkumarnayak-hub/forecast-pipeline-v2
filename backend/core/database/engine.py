...
class Database:
    ...
    def init_database(self):
        ...
        with self.engine.connect() as conn:
            ...
            migrations = [
                ...
                self._migrate_hub_mapping_version_history,
            ]
    ...
    def create_auth_session(
        ...
        *,
        ...
        from sqlalchemy.orm import Session
        from core.database.models import AuthSession
    ...
    def save_baseline_run(self, run_data):
        ...
        from sqlalchemy.orm import Session
        from core.database.models import BaselineRun
    ...
    def get_user_preferences(self, user_id: int) -> dict:
        ...
        from core.security.permissions import DEFAULT_PREFERENCES
    ...
    def log_master_sync(self, sync_data):
        ...
        from sqlalchemy.orm import Session
        from core.database.models import MasterSyncLog
    ...
    def get_master_sync_history(self, master_type=None, limit=50):
        ...
        from sqlalchemy.orm import Session
        from core.database.models import MasterSyncLog, User
    ...
    def _migrate_hub_launch_version_history(self, conn) -> None:
        ...
        if self.backend == "postgresql":
            conn.execute(
                text("""
                    CREATE TABLE IF NOT EXISTS hub_launch_version_history (
    ...
    def get_hub_launch_versions(self, limit: int = 20) -> list[dict]:
        """Fetch history timeline entries of FF Input configuration changes."""
        try:
            with self.engine.connect() as conn:
                rows = conn.execute(
                    text("""
            ...
            import logging
            logging.getLogger(__name__).exception("Failed to fetch hub launch versions from DB")
            return []

    def get_latest_hub_launch_status(self) -> dict:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return {
                    "change_detected": not bool(row[1])  # Alert active if is_dismissed = 0
                }
        except Exception:
            import logging
            logging.getLogger(__name__).exception("Failed to query latest hub launch alert status")
            return {"change_detected": False}

    def dismiss_hub_launch_alerts(self) -> None:
    ...
    def _migrate_hub_mapping_version_history(self, conn) -> None:
        ...
        if self.backend == "postgresql":
            conn.execute(
                text("""
                    CREATE TABLE IF NOT EXISTS hub_mapping_version_history (
                        version_id TEXT PRIMARY KEY,
                        detected_at TIMESTAMPTZ NOT NULL,
                        summary TEXT,
                        diff_json TEXT,
    ...
    def get_hub_mapping_versions(self, limit: int = 20) -> list[dict]:
        """Fetch history timeline entries of Hub_Mapping configuration changes."""
        try:
            with self.engine.connect() as conn:
                rows = conn.execute(
                    text("""
            ...
            import logging
            logging.getLogger(__name__).exception("Failed to fetch hub mapping versions from DB")
            return []

    def get_latest_hub_mapping_status(self) -> dict:
        """Check if any Hub_Mapping change alert is active (undismissed)."""
        try:
            with self.engine.connect() as conn:
                row = conn.execute(
                    text("""
                ...
                if not row:
                    return {"change_detected": False}
                return {"change_detected": not bool(row[1])}
        except Exception:
            import logging
            logging.getLogger(__name__).exception("Failed to query latest hub mapping alert status")
            return {"change_detected": False}

    def dismiss_hub_mapping_alerts(self) -> None:
        """Mark undismissed Hub_Mapping alerts as dismissed."""
        try:
            with self.engine.begin() as conn:
                conn.execute(
                    text("""
