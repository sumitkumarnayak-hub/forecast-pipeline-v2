...
class GoogleSheetsManager:
    ...
    def _persist_dp_logics_table(df: pd.DataFrame, save_path: str) -> None:
        ...
        if Path(save_path).name.lower().startswith("percentile"):
            try:
                from features.baseline.io import write_percentile_engine_sidecars
    ...
    def _sync_dp_logics_parallel(
        ...
        *,
        ...
        from app.config import HUB_LEVEL_PLANNING_SHEET_KEY, SHEETS_CONFIG
    ...
    def _batch_fetch_worksheets(
        ...
        *,
        ...
        from core.shared.sheets_throttle import in_pipeline_mode
    ...
    def read_hub_changes_table(self, *, seed_from_legacy: bool = True) -> pd.DataFrame:
        ...
        """
        from features.hub_launch.sync import normalize_hub_changes_df
    ...
    def read_pipeline_params(self):
        ...
        try:
            from app.config import PIPELINE_PARAMS_SHEET_URL
            if not PIPELINE_PARAMS_SHEET_URL:
