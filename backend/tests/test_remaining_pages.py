...
def test_wizard_context(client, auth_headers):
    with patch("features.product_launch.wizard.list_categories", return_value=["A"]):
        with patch("features.product_launch.wizard.list_cities", return_value=["Mumbai"]):
...
def test_insights_executive_summary(client, auth_headers):
    with patch("features.dashboard.analytics.build_week_analytics", return_value={"kpis": {"x": 1}}):
        with patch("features.dashboard.analytics.list_available_weeks", return_value={"default_week": "Wk 1"}):
