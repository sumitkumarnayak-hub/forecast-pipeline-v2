...
def final_plan_bootstrap(
    ...
    from features.final_plan.engine import (
        get_latest_output_preview,
        load_hub_suggestions_preview,
    )
    from features.final_plan.inputs import (
        get_inputs_status,
...
def final_plan_city_mapping(
    ...
    from features.final_plan.inputs import load_city_mapping_preview
...
def sync_city_mapping(current_user: dict = Depends(require_write)):
    ...
    try:
        from features.final_plan.inputs import sync_city_mapping_to_folder
...
def sync_festive(current_user: dict = Depends(require_write)):
    ...
    try:
        from features.final_plan.inputs import sync_festive_placeholder_from_sheet
...
async def upload_final_plan_input(
    ...
    try:
        from features.final_plan.inputs import save_uploaded_input
...
def sync_adhoc(
    ...
    try:
        from features.final_plan.sync import sync_adhoc_from_sheet
...
def sync_inventory(
    ...
    try:
        from features.final_plan.sync import sync_inventory_from_sheet
...
def run_final_plan(
    ...
    try:
        from features.final_plan.engine import run_final_plan_engine
...
def latest_final_plan_output(
    ...
    from features.final_plan.engine import get_latest_output_preview
...
def hub_suggestions_preview(
    ...
    from features.final_plan.engine import load_hub_suggestions_preview
...
def sync_inv_buffer(current_user: dict = Depends(require_write)):
    ...
    try:
        ...
        import pandas as pd
        from app.config import INV_LOGICS_SHEET_KEY, FF_INV_LOGIC_FOLDER
