...
from core.utils.dataframe import clean_sheet_df, df_to_records

from core.shared.api_cache import CacheNS, cached, cache_invalidate
...
def get_inventory_buffer(current_user: dict = Depends(get_current_user)):
    try:
        from app.config import INV_LOGICS_SHEET_KEY
        gsm = get_sheets_manager()
...
def restore_snapshot(
    ...
    try:
        from core.shared.sync_versioning import restore_master_snapshots_to_sheets, list_snapshot_meta
...
def trigger_master_sync(
    ...
    try:
        from features.master_data.sync import run_master_data_sync
...
def preview_new_hub_sync(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        from features.hub_launch.sync import (
...
def confirm_new_hub_sync(
    ...
    try:
        from features.hub_launch.auto_sync import run_new_hub_launch_sync
